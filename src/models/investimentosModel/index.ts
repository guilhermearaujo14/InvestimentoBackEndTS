class Investimentos {
    ...
    constructor(ID: number, USUARIO_ID: number, TIPO_ATIVO_ID: number, PAPEL: string, QUANTIDADE: number, PRECO_MEDIO: number, TOTAL_INVESTIDO: number){
        this.ID = ID
    ...
    static async GetInvestimentos(ID?: number, USUARIO_ID?: number, TIPO_ATIVO_ID?: number, PAPEL?: string){
        const sql = `
            ...
            WHERE 
                ((ID = ${ID}) OR (${ID} IS NULL) OR (${ID} = '') OR (${ID} = 0)) 
    ...
    async AtualizaInvestimentos(ID: number, QUANTIDADE?: number, PRECO_MEDIO?: number, TOTAL_INVESTIDO?: number){
